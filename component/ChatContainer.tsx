...
function ChatContainer() {
  ...
  return (
    ...
    <div className="flex flex-col h-full">
      <div className="flex-1 overflow-y-auto p-4" style={{ maxHeight: '85vh' }}>
        {messages.map((message, idx) => (
          ...
          >
             <div
              ...
              style={{ maxWidth: 'calc(100% - 20px)', overflowWrap: 'break-word', wordWrap: 'break-word' }}
            >
              {/* Ensure that content is an array before mapping */}
              {Array.isArray(message.content) ? (
                message.content.map((content, index) => {
                  if (content.type === "text") {
                    return <p key={index} style={{ hyphens: 'auto' }}>{content.text}</p>;
